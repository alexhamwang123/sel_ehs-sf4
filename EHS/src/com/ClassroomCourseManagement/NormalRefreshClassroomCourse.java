...
//@Test
@Test
public class NormalRefreshClassroomCourse {
    public void NormalRefreshClassroomCourse() throws IOException, InterruptedException {
...
// Calendar&Time//click save
        ...
        //Check Never expire, refresh  Grays out
        new Select(Expiration).selectByVisibleText("Never Expires");

        //if  refresh btn displays, it fails
        ...
        if(!driver.findElement(By.xpath("//div[@class='prereq col']//div//button[@class='btn btn-primary'][contains(text(),'Select')]")).isDisplayed()){
            Assert.fail("Never expire, refresh  Grays out Failed");

        }
        else{
            System.out.println("Never expire, prereq btn shown");
        ...
        //if prereq or refresh btn does not displays, it fails
        if(!driver.findElement(By.xpath("//small[contains(text(),'Select up to 3 refresh courses. Refresh courses ar')]")).isDisplayed()||
                !driver.findElement(By.xpath("//div[@class='prereq col']//div//button[@class='btn btn-primary'][contains(text(),'Select')]")).isDisplayed()){
            Assert.fail("Never expire, refresh  Grays out Failed");

        }
        else{
            System.out.println("6 months, refresh and prereq btns shown");
